    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-white border-b px-6 py-4 flex justify-between items-center max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/subjects"
            className="text-sm font-semibold text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-lg transition"
          >
            📚 موادي
          </Link>
          <button
            onClick={handleLogout}
            className="text-sm font-semibold text-red-600 hover:bg-red-50 px-3 py-1.5 rounded-lg transition"
          >
            خروج
          </button>
        </div>
      ...
      <main className="max-w-7xl mx-auto w-full p-6 flex-1 space-y-8">
        ...
        {/* Action Cards */}
        <div>
          <h2 className="text-xl font-bold text-slate-800 mb-4">ماذا تحتاج اليوم؟</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
            {[
              { title: "دروسي", icon: "📚", color: "bg-blue-50 text-blue-600", href: "/subjects" },
              { title: "اختبرني", icon: "🧠", color: "bg-purple-50 text-purple-600", href: "/subjects" },
              { title: "حلل صورة", icon: "📸", color: "bg-amber-50 text-amber-600", soon: true },
              { title: "اسأل مساعدي", icon: "🤖", color: "bg-emerald-50 text-emerald-600", href: "/assistant" },
              { title: "تقدمي", icon: "📊", color: "bg-rose-50 text-rose-600", href: "/progress" },
              { title: "خطة اليوم", icon: "🎯", color: "bg-indigo-50 text-indigo-600", href: "/progress" },
            ].map((card, idx) => {
              const inner = (
                <>
                  ...
                  <span className="font-bold text-sm text-slate-700">{card.title}</span>
                  {"soon" in card && card.soon && (
                    <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-lg">قريبًا</span>
                  )}
                </>
              );
              const cls = "bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition text-center flex flex-col items-center justify-center gap-2";
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
          ...
          {planItems.length === 0 ? (
            <p className="text-slate-500 text-sm text-center py-4 leading-relaxed">
            ...
            <div className="space-y-3">
              {planItems.map((item) => (
                <Link
                  ...
                  className="flex items-center gap-3 px-4 py-3 bg-slate-50 rounded-2xl border border-slate-100 hover:shadow-md transition"
                >
                  <span className="text-2xl">
                    {item.kind === "fix_mistake" ? "🔁" : item.kind === "retry_quiz" ? "🧠" : "📚"}
                  </span>
                  <div className="flex-1">
                    <p className="font-bold text-sm text-slate-800">{item.lesson_title}</p>
                    <p className="text-xs text-slate-500">{item.reason}</p>
                  </div>
                  <span className="text-xs font-extrabold text-blue-700 bg-blue-50 px-2 py-1 rounded-lg">
                    {item.priority}
                  </span>
                </Link>
              ))}
              <Link href="/progress" className="block text-center text-sm font-semibold text-blue-600 hover:underline pt-1">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
          ...
          <div className="flex items-center gap-4 mb-5">
            ...
            <span className="font-extrabold text-slate-800">{overallPct}%</span>
          </div>
          <div className="grid grid-cols-2 gap-4 text-center">
            <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
              <div className="text-2xl font-extrabold text-emerald-700">{completedCount}</div>
              <div className="text-xs font-semibold text-emerald-600">دروس مكتملة ✓</div>
            </div>
            <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100">
              <div className="text-2xl font-extrabold text-amber-700">{remaining}</div>
              <div className="text-xs font-semibold text-amber-600">دروس متبقية ○</div>
            </div>
          </div>
        </div>
      </main>
